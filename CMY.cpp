...

{
	...
	unsigned char CMY[SizeL][SizeW][BytesPerPixel];
	...
	for (i=0; i<SizeL; i++)
	{
		for (j=0; j<SizeW; j++)
		{
			...
			{
				val=255-(int)Imagedata[i][j][channel];
				CMY[i][j][channel]=(unsigned char)val;
			}
		}
	}
	...
	fwrite(CMY, sizeof(unsigned char), SizeL*SizeW*BytesPerPixel, file);
	fclose(file);
